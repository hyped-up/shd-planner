...
```
  ╔═══════════════════════════════════════════════════════════════════╗
  ║  S T R A T E G I C   H O M E L A N D   D I V I S I O N         ║
  ║  ─────────────────────────────────────────────────────────────── ║
...
| Google Drive | `STANDBY` | Optional OAuth-based backup/restore |
...
# ISAC online at http://localhost:3000
```
...
No database required. No external APIs required. No environment variables required for core functionality. The app is fully self-contained with all game data bundled at build time.
...
TOTAL: 341 entities indexed  //  LAST UPDATE: TU21.1
```
...
│                                                              │
│  ┌──────────┐  ┌───────────┐  ┌───────────┐  ┌──────────┐  │
│  │  Builder  │  │  Database  │  │ DPS Calc  │  │ Sharing  │  │
│  │  Planner  │  │  Browser   │  │  Engine   │  │  Codec   │  │
│  └────┬─────┘  └─────┬─────┘  └─────┬─────┘  └────┬─────┘  │
│       │              │              │              │         │
│  ┌────┴──────────────┴──────────────┴──────────────┴─────┐  │
...
└─────────────────────────────────────────────────────────────┘
                    ...
                    Data Pipeline
                           │
┌─────────────────────────────────────────────────────────────┐
│              SHD PLANNER MCP SERVER (Python)                  │
│                                                              │
│   Canonical JSON data ──> import-mcp-data.ts ──> Web App     │
...
The Dockerfile uses a multi-stage build (deps -> build -> runner) with Alpine Linux, runs as non-root user `nextjs`, and produces a ~400MB image with standalone output.
...
- [Google Drive Setup](docs/GOOGLE-DRIVE-SETUP.md) — OAuth configuration guide

---

```
  ISAC: "Build optimization complete, Agent. Stay safe out there."
```
